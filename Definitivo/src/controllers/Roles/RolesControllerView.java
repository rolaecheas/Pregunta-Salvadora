			...
			if (uSearch.isEmpty()) {
				...
				if (rSearch.isEmpty()) {
					...
					if (aSearch.isEmpty()) {
						...
						Rol rol = pm.getObjectById(Rol.class, Long.parseLong(req.getParameter("id")));
						req.setAttribute("rol", rol);

						String query0 = "SELECT FROM " + Access.class.getName();
						List<Access> access = (List<Access>) pm.newQuery(query0).execute();
						...
						RequestDispatcher rd = this.getServletContext()
								.getRequestDispatcher("/WEB-INF/Views/Roles/view.jsp");
						rd.forward(req, resp);
					}
				}
			}
